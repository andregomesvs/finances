...
A fundação inclui dashboard responsivo, tema claro/escuro, Design Tokens e integração server-side com Firestore. Os dados visíveis no dashboard ainda são demonstrativos; autenticação e gravação pelos formulários entram nas próximas etapas.
...
- Não use dados reais antes da implementação da autenticação.
